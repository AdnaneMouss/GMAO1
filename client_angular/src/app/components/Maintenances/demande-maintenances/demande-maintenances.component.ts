...
@Component({
  selector: 'app-demande-maintenances',
  templateUrl: './demande-maintenances.component.html',
  styleUrl: './demande-maintenances.component.css'
})
export class DemandeMaintenancesComponent implements OnInit {
    ...
    this.maintenanceCorrectiveService.getAllMaintenances()
      .subscribe(
        (data: MaintenanceCorrective[]) => {
          const relevantStatuses = ['DEMANDEE', 'EN_ATTENTE', 'EN_COURS'];
  ...
  addMaintenance(): void {
    ...
    this.newMaintenance.demandeeParNom = this.currentUser?.nom || '';
    this.newMaintenance.demandeeParId = this.currentUser?.id || 0;

    if (this.currentUser) {
      console.log('User Nom:', this.currentUser.nom);
    } else {
      console.log("No user logged in, 'demandeeParNom' will not be set.");
    }
    ...
    this.maintenanceCorrectiveService.requestMaintenance(this.newMaintenance)
      .subscribe(
        (addedMaintenance) => {
  ...
  updateMaintenance(): void {
    ...
    // Ensure both creeParNom and creeParId are set
    this.setCurrentUser();
    this.selectedMaintenance.demandeeParNom = this.currentUser?.nom || '';
    this.selectedMaintenance.demandeeParId = this.currentUser?.id || 0;
    ...
    this.maintenanceCorrectiveService.updateMaintenanceCorrectiveLambda(
