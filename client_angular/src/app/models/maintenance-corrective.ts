...
export interface MaintenanceCorrective {
  ...
  titre: string;
  description: string;
  statut: 'DEMANDEE' | 'EN_ATTENTE' | 'EN_COURS' | 'TERMINEE' | 'ANNULEE';
  priorite: 'FAIBLE' | 'NORMALE' | 'URGENTE';
  dateCreation: string; // ISO 8601 format date string
  dateDemande?: string; // ISO 8601 format date string
  ...
  demandeeParId: number;
  demandeeParNom: string; // ID of the user assigned to the ticket
